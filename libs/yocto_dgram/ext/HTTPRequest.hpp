     ...
     public:
      ...
      explicit Socket(const InternetProtocol internetProtocol)
        ...
        if (flags == -1) {
          close();
          throw std::system_error(
              errno, std::system_category(), "Failed to get socket flags");
        }

        if (fcntl(endpoint, F_SETFL, flags | O_NONBLOCK) == -1) {
          close();
          throw std::system_error(
              errno, std::system_category(), "Failed to set socket flags");
        }
#endif  // defined(_WIN32) || defined(__CYGWIN__)
        ...
        if (setsockopt(endpoint, SOL_SOCKET, SO_NOSIGPIPE, &value,
          ...
          close();
          throw std::system_error(
              errno, std::system_category(), "Failed to set socket option");
        }
#endif  // __APPLE__
      }
      ...
      void connect(const struct sockaddr* address, const socklen_t addressSize,
        ...
        if (result == -1) {
          if (WSAGetLastError() == WSAEWOULDBLOCK) {
            ...
            if (socketError != 0)
              throw std::system_error(
                  socketError, std::system_category(), "Failed to connect");
          } else
            throw std::system_error(
                WSAGetLastError(), std::system_category(), "Failed to connect");
        }
#else
        auto result = ::connect(endpoint, address, addressSize);
        ...
        if (result == -1) {
          if (errno == EINPROGRESS) {
            ...
            if (getsockopt(endpoint, SOL_SOCKET, SO_ERROR, &socketError,
                    &optionLength) == -1)
              throw std::system_error(
                  errno, std::system_category(), "Failed to get socket option");
      ...
      std::size_t send(const void* buffer, const std::size_t length,
        ...
        if (result == -1)
          throw std::system_error(
              WSAGetLastError(), std::system_category(), "Failed to send data");
#else
        auto result = ::send(
      ...
      std::size_t recv(
          void* buffer, const std::size_t length, const std::int64_t timeout) {
        ...
        if (result == -1)
          throw std::system_error(
              WSAGetLastError(), std::system_category(), "Failed to read data");
#else
        auto result = ::recv(
        ...
        if (state != State::parsingBody)
          for (;;) {
            ...
            if (line.empty()) {
              ...
              break;
            } else if (state == State::parsingStatusLine)  // RFC 7230, 3.1.2.
                                                           // Status Line
            {
              state = State::parsingHeaders;

              const auto httpEndIterator = std::find(
                  line.begin(), line.end(), ' ');

              if (httpEndIterator != line.end()) {
                const auto statusStartIterator = httpEndIterator + 1;
                const auto statusEndIterator   = std::find(
                      statusStartIterator, line.end(), ' ');
                const std::string status{
                    statusStartIterator, statusEndIterator};
                response.status = std::stoi(status);

                if (statusEndIterator != line.end()) {
                  const auto descriptionStartIterator = statusEndIterator + 1;
                  response.description                = std::string{
                      descriptionStartIterator, line.end()};
                }
              }
            } else if (state ==
                       State::parsingHeaders)  // RFC 7230, 3.2. Header Fields
            {
              ...
              // ltrim
              headerValue.erase(
                  headerValue.begin(), std::find_if(headerValue.begin(),
            ...
            for (;;) {
              if (expectedChunkSize > 0) {
                ...
                if (removeCrlfAfterChunk) {
                  ...
                  if (!std::equal(
                          crlf.begin(), crlf.end(), responseData.begin()))
                    throw ResponseError("Invalid chunk");

                  removeCrlfAfterChunk = false;
                  responseData.erase(
                      responseData.begin(), responseData.begin() + 2);
                }

                const auto i = std::search(responseData.begin(),
                    responseData.end(), crlf.begin(), crlf.end());
